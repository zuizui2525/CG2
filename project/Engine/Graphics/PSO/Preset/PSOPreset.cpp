...
PSOPreset PSOPreset::CreateVignettePreset(
    ID3D12Device* device,
    IDxcUtils* dxcUtils,
    IDxcCompiler3* dxcCompiler,
    IDxcIncludeHandler* includeHandler) {
    ...
    preset.rootSignature = rs.Build(device);
    assert(preset.rootSignature && "Vignette RootSignature creation failed!");
    ...
    assert(vsResult && "Vignette VS Compile Failed!");

    bool psResult = preset.shaderProgram.CompilePS(
        L"resources/Shader/Fullscreen/Vignette.PS.hlsl",
        dxcUtils, dxcCompiler, includeHandler
    );
    assert(psResult && "Vignette PS Compile Failed!");

    return preset;
}

PSOPreset PSOPreset::CreatePostProcessPreset(
    ID3D12Device* device,
    IDxcUtils* dxcUtils,
    IDxcCompiler3* dxcCompiler,
    IDxcIncludeHandler* includeHandler) {
    ...
    preset.rootSignature = rs.Build(device);
    assert(preset.rootSignature && "PostProcess RootSignature creation failed!");
    ...
    assert(vsResult && "PostProcess VS Compile Failed!");

    bool psResult = preset.shaderProgram.CompilePS(
        L"resources/Shader/Fullscreen/PostProcess.PS.hlsl",
        dxcUtils, dxcCompiler, includeHandler
    );
    assert(psResult && "PostProcess PS Compile Failed!");
