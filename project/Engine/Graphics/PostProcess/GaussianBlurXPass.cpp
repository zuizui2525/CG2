...
void GaussianBlurXPass::Initialize(ID3D12Device* device) {
    ...
    assert(paramsResource_ != nullptr && "Failed to create paramsResource in GaussianBlurXPass!");

    HRESULT hr = paramsResource_->Map(0, nullptr, reinterpret_cast<void**>(&paramsData_));
    assert(SUCCEEDED(hr) && paramsData_ != nullptr && "Failed to map paramsResource in GaussianBlurXPass!");
    ...
    assert(rootSignature_ && "GaussianBlurXPass RootSignature creation failed!");
    ...
    assert(vsResult && "GaussianBlurXPass VS Compile Failed!");

    bool psResult = shaderProgram_.CompilePS(
        L"resources/Shader/Fullscreen/GaussianBlurX.PS.hlsl",
        dxCommon->GetDxcUtils(), dxCommon->GetDxcCompiler(), dxCommon->GetIncludeHandler()
    );
    assert(psResult && "GaussianBlurXPass PS Compile Failed!");
    ...
    assert(SUCCEEDED(hr) && "Failed to create GaussianBlurXPass GraphicsPipelineState!");
}

void GaussianBlurXPass::Draw(ID3D12GraphicsCommandList* commandList, D3D12_GPU_DESCRIPTOR_HANDLE inputSRV) {
