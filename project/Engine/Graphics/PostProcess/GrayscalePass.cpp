...
void GrayscalePass::Initialize(ID3D12Device* device) {
    ...
    assert(rootSignature_ && "GrayscalePass RootSignature creation failed!");
    ...
    assert(vsResult && "GrayscalePass VS Compile Failed!");

    bool psResult = shaderProgram_.CompilePS(
        L"resources/Shader/Fullscreen/Grayscale.PS.hlsl",
        dxCommon->GetDxcUtils(), dxCommon->GetDxcCompiler(), dxCommon->GetIncludeHandler()
    );
    assert(psResult && "GrayscalePass PS Compile Failed!");
    ...
    assert(SUCCEEDED(hr) && "Failed to create GrayscalePass GraphicsPipelineState!");
}

void GrayscalePass::Draw(ID3D12GraphicsCommandList* commandList, D3D12_GPU_DESCRIPTOR_HANDLE inputSRV) {
