...
void SepiaPass::Initialize(ID3D12Device* device) {
    ...
    assert(rootSignature_ && "SepiaPass RootSignature creation failed!");
    ...
    assert(vsResult && "SepiaPass VS Compile Failed!");

    bool psResult = shaderProgram_.CompilePS(
        L"resources/Shader/Fullscreen/Sepia.PS.hlsl",
        dxCommon->GetDxcUtils(), dxCommon->GetDxcCompiler(), dxCommon->GetIncludeHandler()
    );
    assert(psResult && "SepiaPass PS Compile Failed!");
    ...
    assert(SUCCEEDED(hr) && "Failed to create SepiaPass GraphicsPipelineState!");
}

void SepiaPass::Draw(ID3D12GraphicsCommandList* commandList, D3D12_GPU_DESCRIPTOR_HANDLE inputSRV) {
